document.addEventListener('DOMContentLoaded', function () {
 // Định nghĩa mảng với các giá trị start và end tương ứng cho từng chữ cái
  ...
  let WORD_NUMBER = 12;
  let addedImage = [];
  const pathSound = "assets/alphabet/ocpd_01_the_alphabet.mp3";
  const pathImg = "assets/alphabet/";
  ...
  // Hàm để tải hình ảnh vào canvas
    function addImagesToCanvas(resize=false) {
      ...
      remainingWords.forEach((word) => {
          ...
          let path = `${pathImg}${word.letter}.webp`;
              ...
              function handleImageClick(image, word) {
                  if (word.letter === currentWord.letter) {
                      ...
                      if (remainingWords.length > 0) {
                          setTimeout(() => {
                              playSound();
                          }, 1000 / 3); // Phát âm từ mới sau khi thông báo
                      } else {
                          setTimeout(() => {
                              showFireworks();
